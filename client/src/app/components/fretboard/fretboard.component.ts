...
export class FretboardComponent implements OnInit, OnDestroy {
  ...
  playMode(): void {
    ...
    const state = this.state;
    const keyIndex = this.musicTheoryService.getNoteIndex(state.selectedKey);
    const scaleIntervals = currentItem.intervals;
    const scaleLength = scaleIntervals.length; // 7 for most scales, 5 for pentatonic, etc.

    // Build the scale note values (0-11) from intervals
    const scaleNoteValues = scaleIntervals.map((interval: number) => (keyIndex + interval) % 12);

    type NotePosition = { stringIndex: number; fretIndex: number; noteName: string; octave: number; noteValue: number; scaleDegree: number };

    // Collect all scale notes within a hand position (4 frets: index, middle, ring, pinky)
    const findNotesInPosition = (startFret: number): NotePosition[] => {
      const maxFret = startFret + 3; // 4 frets total (e.g., frets 3,4,5,6)
      const notes: NotePosition[] = [];

      // Strings are ordered high to low (G=0, D=1, A=2, E=3 for bass)
      for (let stringIdx = 0; stringIdx < this.fretboard.length; stringIdx++) {
        const stringNotes = this.fretboard[stringIdx];

        for (let fretIdx = startFret; fretIdx <= maxFret && fretIdx < stringNotes.length; fretIdx++) {
          const note = stringNotes[fretIdx];
          const scaleDegree = scaleNoteValues.indexOf(note.noteValue);
          if (note.isInMode && scaleDegree !== -1) {
            notes.push({
    ...
    // Build scale by going string by string (low to high pitch), playing all notes on each string
    // in fret order before moving to the next string
    const buildScaleSequence = (notes: NotePosition[]): NotePosition[] => {
      if (notes.length === 0) return [];

      // Group notes by string
      const notesByString: Map<number, NotePosition[]> = new Map();
      for (const note of notes) {
        if (!notesByString.has(note.stringIndex)) {
          notesByString.set(note.stringIndex, []);
        }
        notesByString.get(note.stringIndex)!.push(note);
      }

      // Sort notes on each string by fret
      for (const [, stringNotes] of notesByString) {
        stringNotes.sort((a, b) => a.fretIndex - b.fretIndex);
      }

      // Find the lowest root note to start from
      const rootNotes = notes.filter(n => n.scaleDegree === 0);
      if (rootNotes.length === 0) return [];

      // Sort roots by pitch to find the lowest
      rootNotes.sort((a, b) => (a.octave * 12 + a.noteValue) - (b.octave * 12 + b.noteValue));
      const startNote = rootNotes[0];
      const startOctave = startNote.octave;

      // Build the sequence: collect notes in pitch order, but prefer staying on same string
      const sequence: NotePosition[] = [];
      const usedPositions = new Set<string>();

      // Start with the root
      sequence.push(startNote);
      usedPositions.add(`${startNote.stringIndex}-${startNote.fretIndex}`);

      // For each subsequent scale degree
      for (let degree = 1; degree <= scaleLength; degree++) {
        const targetDegree = degree % scaleLength;
        const targetOctave = degree >= scaleLength ? startOctave + 1 : startOctave;
        const targetNoteValue = scaleNoteValues[targetDegree];

        // Find all candidates for this scale degree
        const candidates = notes.filter(n =>
          n.noteValue === targetNoteValue &&
          n.octave === targetOctave &&
          !usedPositions.has(`${n.stringIndex}-${n.fretIndex}`)
        );
        ...
        // Sort candidates: prefer same string, then adjacent string moving toward higher pitch strings
        candidates.sort((a, b) => {
          // Priority 1: Same string
          if (a.stringIndex === lastNote.stringIndex && b.stringIndex !== lastNote.stringIndex) return -1;
          if (b.stringIndex === lastNote.stringIndex && a.stringIndex !== lastNote.stringIndex) return 1;

          // Priority 2: Moving toward higher strings (lower stringIndex) - natural scale direction
          // But prefer staying on lower strings (higher stringIndex) if on different strings
          if (a.stringIndex !== b.stringIndex) {
            // Prefer the string that's one step higher (lower index) from current
            const aStep = lastNote.stringIndex - a.stringIndex;
            const bStep = lastNote.stringIndex - b.stringIndex;
            // Prefer moving up by 1 string, then staying, then bigger jumps
            if (aStep === 1 && bStep !== 1) return -1;
            if (bStep === 1 && aStep !== 1) return 1;
            if (aStep === 0 && bStep !== 0) return -1;
            if (bStep === 0 && aStep !== 0) return 1;
            return aStep - bStep;
          }

          // Priority 3: Higher fret on same string (ascending)
          return a.fretIndex - b.fretIndex;
        });

        sequence.push(candidates[0]);
        usedPositions.add(`${candidates[0].stringIndex}-${candidates[0].fretIndex}`);
      }

      return sequence;
    };

    // Try hand positions starting from fret 0, find the first that gives a complete scale
    let bestPosition: NotePosition[] = [];
    let bestStartFret = 0;
    const targetLength = scaleLength + 1; // Full scale + octave

    for (let startFret = 0; startFret <= 12; startFret++) {
      const notesInPosition = findNotesInPosition(startFret);
      const sequence = buildScaleSequence(notesInPosition);

      if (sequence.length >= targetLength) {
        bestPosition = sequence.slice(0, targetLength);
        bestStartFret = startFret;
        break;
      } else if (sequence.length > bestPosition.length) {
        bestPosition = sequence;
        bestStartFret = startFret;
      }
    }

    if (bestPosition.length === 0) return;

    const notesToPlay = bestPosition;
    console.log(`Hand position: frets ${bestStartFret}-${bestStartFret + 3}`);

    // Debug: Log the notes that will be played with string/fret positions
    console.log('Scale notes to play:', notesToPlay.map((n) => {
      const strName = this.fretboard[n.stringIndex]?.[0]?.noteName || `S${n.stringIndex}`;
      return `${n.noteName}(${strName}${n.fretIndex})`;
    }).join(' '));
